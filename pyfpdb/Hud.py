...
#        self.topify_window(self.main_window)
    ...
    def update_table_position(self):
        ...
        if self.table.gdkhandle is not None:
            ...
            log.debug("update_table_position:"+str(oldx)+","+str(oldy))
...
#    move the stat windows
        ...
        log.debug("up_update_table_position:"+str(self.table.oldx)+","+str(self.table.oldy))
        adj = self.adj_seats(self.hand, self.config)
        loc = self.config.get_locations(self.table.site, self.max)
        for i, w in enumerate(self.stat_windows.itervalues()):
            (x, y) = loc[adj[i+1]]
...
#    also kill any aux windows
    ...
    def resize_windows(self, *args):
        log.debug("resize_windows old:"+str(self.table.oldwidth)+"x"+str(self.table.oldheight))
        log.debug("resize_windows new:"+str(self.table.width)+"x"+str(self.table.height))
