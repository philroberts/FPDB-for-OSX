...
class MergeSummary(TourneySummary):
    ...
    re_HTMLGameType = re.compile("""Game Type</th><td>(?P<LIMIT>Fixed Limit|No Limit|Pot Limit) (?P<GAME>Texas\sHoldem|Omaha|Omaha\sHiLo|2\-7\sLow\sTriple\sDraw|Badugi|Seven\sCard\sStud|Seven\sCard\sStud\sHiLo|Five\sCard\sStud|Razz|HORSE|HA|HO)</td><h1>""")
    ...
    def parseSummaryFromHH(self, mg):
        ...
        if tablename not in hhc.SnG_Structures:
            log.error(_("MergeSummary.determineGameType: No match in SnG_Structures"))
            raise FpdbParseError
        
        self.buyin     = int(100*hhc.SnG_Structures[tablename]['buyIn'])
        self.fee       = int(100*hhc.SnG_Structures[tablename]['fee'])
        self.entries   = hhc.SnG_Structures[tablename]['seats']
        self.currency  = hhc.SnG_Structures[tablename]['payoutCurrency']
        self.prizepool = sum(hhc.SnG_Structures[tablename]['payouts'])
        payouts = len(hhc.SnG_Structures[tablename]['payouts'])
        self.isSng     = True
        
        if hhc.SnG_Structures[tablename]['multi']:
        ...
        for handText in handsList:
            ...
            if out:
                ...
                for n in out:
                    ...
                    if n in won:
                        rank = 1
                        winnings = int(100*hhc.SnG_Structures[tablename]['payouts'][0])
                    else:
                        rank = len(players) - i
                        if rank <= payouts:
                            winnings = int(100*hhc.SnG_Structures[tablename]['payouts'][rank-1])
                        i += 1
